<template>
  <li v-if="!childrenLinks && isHeader" :class="{headerLink: true, className}">
    <router-link :to="link" class="sidebar-link">
      ...
      <span class="icon" v-else>
        ...
        <i :class="fullIconName" style="font-size: 23px;
    opacity: 0.5;"></i>
    
        <!-- <img :src="logo" alt=""> -->
      </span>
      {{header}} <sup v-if="label" :class="'text-' + labelColor" class="headerLabel poppins">{{label}}</sup>
  ...
  <li v-else-if="childrenLinks" :class="{headerLink: true, className}">
    ...
    opacity: 0.5;"></i>
        ...
        {{header}} <sup v-if="label" :class="'text-' + labelColor" class="ml-1 headerLabel poppins">{{label}}</sup>
...
import TIcon from '@/assets/icons/iconTransaction.vue'
